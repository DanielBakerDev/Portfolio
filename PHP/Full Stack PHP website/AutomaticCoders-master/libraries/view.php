...
class View extends Functions
{
    ...
    public function render($name, $args = array())
    {
        ...
        if (file_exists('mvc/' . $newUrl . '/views/' . $name . '.phtml')) {
            require 'mvc/' . $newUrl . '/views/' . $name . '.phtml';
        } else {
            die('File: "mvc/' . $newUrl . '/views/' . $name . '.phtml" does not exist!');
        }
    }
    ...
    public function loadPage()
    {
        ...
        $bundleMod = $this->getBundle('mvc/' . $equrl[0] . '/config/bundle.config.php');
    ...
    public function resBuild(array $files, $module = false)
    {
