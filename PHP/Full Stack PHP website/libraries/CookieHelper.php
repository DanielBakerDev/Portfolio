...
class CookieHelper
{
    const SITE_USER = 'site_user';

    //Create a cookie
    public function getCookie()
    {
        return isset($_COOKIE[self::SITE_USER]) ?? $_COOKIE[self::SITE_USER]; // 86400 = 1 day
    }

    //Create a cookie
    public function setCookie($cookie_value, $time, $path)
    {
        setcookie(self::SITE_USER, $cookie_value, $time, $path); // 86400 = 1 day
    }
